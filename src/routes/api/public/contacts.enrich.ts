...
async function runPipeline(businessId: string, jobId: string, businessName: string, website: string | null, leadId: string | null) {
  ...
  // Step 3: bulk email finder (snipercoder primary → HarvestAPI secondary
  // → anchor~linkedin-to-email tertiary) via shared pipeline helper.
  const emailRes = await runEmailsStep({
    jobId,
    steps,
    businessId,
    dms: insertedDMs.map((d) => ({
      id: d.id as string,
      person_profile_url: (d.person_profile_url as string | null) ?? null,
    })),
  });
  steps = emailRes.steps;
